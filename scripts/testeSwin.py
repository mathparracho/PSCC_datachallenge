...
train_loader = DataLoader(train_ds,num_workers=4)

val_ds = Dataset(data=val_files, transform=val_transforms)
val_loader = DataLoader(val_ds,num_workers=4)
...
"""
model = SwinUNETR(img_size=(192,192,192), in_channels=1, out_channels=2, use_checkpoint=True,drop_rate=0.2).to(device)
...
for epoch in range(max_epochs):
    ...
    save_ckp(checkpoint, True, "./models/model50SwinCkp.pt", "./models/model50Swin.h5")
    
    #via das duvidas se bugar o checkpoint...
    model.eval()
    torch.save(model, "model50Swin.h5")
