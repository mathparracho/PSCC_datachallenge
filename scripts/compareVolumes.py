...
for i in range (1,10):
    img = nib.load(f"./data/test/TRAINpredictionsSWIN12/LUNG1-00{i}_vol/LUNG1-00{i}_vol.nii.gz")
    label = nib.load(f"/tsi/data_education/data_challenge/train/seg/LUNG1-00{i}_seg.nii.gz")
    data = img.get_fdata()
    (
        recist,
          predicted_volume,
    ...
    data2 = label.get_fdata()
    (
        recist2,
          predicted_volume2,
            largest_circle,
            largest_slice,
        ) = find_largest_containing_circle(data2, label.header["pixdim"][1:4])

    print("############################")
    print("racista pred",recist)
    print("racista label",recist2)
    print("volume pred", predicted_volume)
    print("volume label", predicted_volume2)
